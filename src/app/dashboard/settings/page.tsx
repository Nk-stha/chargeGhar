  ...
  const fetchCoupons = async (page = 1) => {
    try {
      ...
      const response = await axios.get(
        `/api/admin/coupons?page=${page}&page_size=${pageSize}`,
      );
                ...
                <td>
                  <span
                    className={`${styles.status} ${
                      pkg.active ? styles.active : styles.inactive
                    }`}
                  >
            ...
            {coupons.map((c) => (
              <tr key={c.id}>
                ...
                <td>
                  <span
                    className={`${styles.status} ${
                      c.status === "active" ? styles.active : styles.inactive
                    }`}
                  >
                    {c.status}
                  </span>
            ...
            {achievements.map((a) => (
              <tr key={a.id}>
                ...
                <td>
                  <span
                    className={`${styles.status} ${
                      a.active ? styles.active : styles.inactive
                    }`}
                  >
        ...
        {selectedSection === "password" && (
          <form
            onSubmit={(e) => {
              ...
              const newPass = (
                e.currentTarget.elements.namedItem("new") as HTMLInputElement
              ).value;
              const confirm = (
                e.currentTarget.elements.namedItem(
                  "confirm",
